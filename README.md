...
npm -w @uniclub/backend run seed:weekly-event
```
...
#### 4. Weekly Event (Sự kiện tuần)

Seed dữ liệu cấu hình mặc định, đề mẫu (cho khối 1-12) và sự kiện nháp cho game Sự kiện tuần:

```bash
npm -w @uniclub/backend run seed:weekly-event
```

### Chạy giả lập kiểm thử tải (Load Test Simulation)

Để kiểm chứng hiệu năng và độ chính xác của cơ chế chấm điểm hàng loạt (Batch Grading) cho game Sự kiện tuần dưới tải giả lập 10,000 học sinh:

```bash
npm -w @uniclub/backend run test:batch-grading
```
...
# Seed weekly-event
docker compose exec backend npm run seed:weekly-event
```

Hoặc chạy tất cả:

```bash
docker compose exec backend sh -c "npm run seed:admin && npm run seed:bots && npm run seed:questions && npm run seed:weekly-event"
```
...
interface WebViewGameEndedPayload {
  ...
  gameType: GameType;          // 'mind_game' | 'quiz_arena' | 'boss_battle' | 'weekly_event'
  kafkaGameType: KafkaGameType;// 'SO_TAI' | 'CARO' | 'LAT_MANH_GHEP' | 'SAN_BOSS' | 'SU_KIEN_TUAN'
  ...
  correctCount?: number;       // Số câu đúng (So Tài, Săn Boss, Sự kiện tuần)
  totalQuestions?: number;     // Tổng số câu hỏi (So Tài, Săn Boss, Sự kiện tuần)
...
| Sự kiện tuần | `weekly_event` | `SU_KIEN_TUAN` | — |
...
| Sự kiện tuần | Màn hình kết quả cá nhân hiện ra | `true` | Gửi khi `personalResult` available từ socket event |
