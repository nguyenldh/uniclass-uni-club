...
import { parseQuestionsFromExcel, generateExcelTemplate, type ParseResult } from '../utils/excel';
...
export function ImportExcelModal({ open, onCancel, onSuccess }: ImportExcelModalProps) {
  ...
  const handleImport = async () => {
    ...
    setImporting(true);
    try {
      const result = await questionService.bulkUpsert(parseResult.rows);
      const parts: string[] = [];
  ...
  const previewColumns = [
    {
      ...
      render: (id: string | undefined) =>
        id ? (
          <Text type="secondary" style={{ fontSize: 12 }}>{id.slice(-6)}</Text>
        ) : (
  ...
  return (
    <Modal
      ...
      width={800}
      footer={[
        <Button key="template" icon={<DownloadOutlined />} onClick={generateExcelTemplate}>
          Tải template mẫu
        </Button>,
